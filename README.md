...
英語が読めなくても、海外掲示板（Reddit）や YouTube で盛り上がったスレ・動画を、現地ファンの
生のコメントつきで日本語まとめにして読めるサイト。対象は **MLB・ボクシング・MMA（UFC・RIZIN）**
の3競技。
...
`boxing` / `mma`）。YouTube コメントを取り込む場合は `scripts/fetch-youtube.mjs`
（要 `YOUTUBE_API_KEY`）。
...
- `scripts/` … Reddit / YouTube 取得スクリプトと手順メモ
