...
**🏆 ポストシーズンは全試合を個別記事にする（2026-09-26 村山決定）**: ギャップ表の行に `postseason`
（ワイルドカードシリーズ／地区シリーズ／リーグ優勝決定シリーズ／ワールドシリーズ）が付いた試合は、
**日本人・ライバルが出ていなくても1試合1本**作る。レギュラーシーズンの2つの取り決めより優先する:
「日本人が絡む試合は日次記事（jp-daily）に集約」（2026-07-30）と「クラウドの個別記事はライバルだけの試合」。
- なぜ: ポストシーズンは1試合ごとに「ドジャース 地区シリーズ 第1戦 海外の反応」の検索が立ち、`/postseason`
  ハブもラウンドごとの棚に記事を並べる。日次記事は「きょうの日本人選手」＝狙う検索が違う。
- ギャップ表はポストシーズンの試合を**全部**載せる（日本人ゼロの試合は `出場: （日本人・ライバルなし）`）。
  `existingArticle` の判定から日次記事は外してある＝日次記事があっても試合記事は `null` のまま出る。
- id は `suggestedId` そのまま（`2026-09-30-padres-vs-cubs-wc-g1`＝ラウンドと第何戦が末尾に付く）。
- タイトルは matome R8 の「ポストシーズンの試合」型、タグは R11 の5（`ポストシーズン`＋`postseason.roundJa`）。
- **同じ動画を日次記事と共有してよいが、コメントは重ねない**。後から作る方が先の記事で使っていない
  コメントを選び、保存前に `node scripts/check-comment-overlap.mjs <この記事id> --with <同じ日の jp-daily の id>`
  が ✓ になることを確認する（2本がほぼ同じ中身になると両方の評価が落ちる）。
- 二重作成ガードの videoId 検索は日次記事を除いて見る:
  `grep -rl "<videoId>" data/threads/mlb/ | grep -v -- '-jp-daily.json'`
- 試合結果ボックス（Step 5 の `backfill-games`）はポストシーズンの試合では勝敗・地区順位・本塁打の号数を
  書かない（API の値がシリーズの勝敗・レギュラーシーズンの確定値・ポストシーズン通算になるため）。本文でも
  「今季◯号」と書かない（成績ボックスの `note` の「ポストシーズン◯号」をそのまま使う）。
