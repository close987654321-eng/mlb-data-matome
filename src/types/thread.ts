...
/**
 * 1試合の決着（格闘技記事のみ）。検索結果用タイトルの材料＝表示には使わない。
 *
 * なぜ持つか（2026-08-12 GSC実測）: 「朝倉海 結果」で順位65、「rizin 朝倉未来 試合結果」で56＝
 * 記事はあるのに結果クエリで拾えていない。編集タイトル（「後藤丈治がテミロフに1R一本勝ち、
 * ファン『マジですげえわ』」）は読み物としては良いが、**クエリの語「結果」と「vs」が無い**。
 * MLB の Thread.game → gameSeo と同じ関係で、この構造化データから検索結果用タイトルだけを
 * 組み直す（src/lib/fightSeo.ts）。記事本文の見出しは触らない。
 *
 * ⚠️ 入れてよいのは記事本文（title/summaryJa）に**明記されている事実だけ**。外部知識で
 * 補完しない＝わからない項目は持たない（CLAUDE.md §4.4）。
 */
export type ThreadFight = {
  /** 対戦者（カード表記の並び。勝敗とは無関係） */
  aJa: string;
  bJa: string;
  /** 勝者（aJa / bJa のどちらかと完全一致）。引き分け・無効試合・不明なら持たない */
  winnerJa?: string;
  /** 決着（「KO」「TKO」「一本」「判定」「判定3-0」など本文の表記どおり） */
  methodJa?: string;
  /** 決着ラウンド（本文に「◯R」の明記があるときだけ） */
  round?: number;
  /** 興行名（「RIZIN.54」「UFC 330」など本文に明記があるときだけ） */
  eventJa?: string;
};
...
/**
 * ④ きょうの現地ざわつき — その日コメント欄が騒いだ話題（日本人と無関係でも可）。
 *
 * `result` を書くと、その話題は**特定の1試合の話**として扱われ、チームLPの試合タイムライン
 * （src/lib/teamGames.ts）がここの引用をその試合の「現地ファンの声」として拾う。日本人が絡まない
 * 試合はまとめ記事が作られないので、日次記事のこの枠が唯一のコメント源になる（2026-08-07）。
 * 話題が試合と結びつかないとき（移籍・論争・シーズン全体の話）は書かない＝紐づけない。
 */
export type DailyBuzz = {
  title: string; // 話題の見出し（例: "「大谷は歩かせてもいいのか」現地で再燃した敬遠論争"）
  media?: ThreadMedia; // 話題の出どころの動画（主役の試合と別ならここに）
  /** その話題の試合結果1行。書式は DailyShort.result と同じ「{自軍} {自軍得点}-{相手得点} {相手} ○/●」 */
  result?: string;
  blocks: StoryBlock[];
};
...
export type ThreadDaily = {
  ...
  hero: DailyHero; // ②
  shorts: DailyShort[]; // ③
  // ④（その日騒ぎが無ければ省く）。1〜2件＝2件目は「日本人が絡まない試合」を優先して拾う枠で、
  // チームLPのタイムラインに現地の声が乗る試合を増やす役目を兼ねる（2026-08-07 に単数から配列化）。
  buzz?: DailyBuzz[];
...
export type Thread = {
  ...
  // title.ja が【海外の反応】で終わらない英語ソース記事）はこのフラグが無くても従来どおり付与される＝
  // 既存ページの表示は変えない。新規の日本語ソース記事だけがこの旗を立てる。
  ...
  fight?: ThreadFight; // 1試合の決着（格闘技記事のみ）。検索結果用タイトルの材料＝fightSeo.ts が使う
