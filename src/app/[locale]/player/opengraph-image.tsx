...
/**
 * /player ハブ（日本人選手 成績まとめ・比較）の OG カード。個別選手 OG と同じ紙面ファミリー
 * （ぼかし観客＋グラウンド背景＋黒レイヤー・src/lib/ogCard）に揃える。違いは主役で、個別は
 * 「選手名＋巨大成績」、ハブは「一覧の表紙＝大判タイトル＋掲載選手の並び」。数値は捏造せず
 * スナップショット由来の年・時点・実在の掲載選手のみ。
 */
...
export default async function Image({ params }: { params: { locale: Locale } }) {
  const { locale } = params;
  const en = locale === 'en';
  const [fonts, bg, all, snap] = await Promise.all([
    loadOgFonts(),
    loadOgBg(),
    getAllThreads(),
    getPlayersSnapshot(),
  ...
  const header = (
    <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
      <div style={{ display: 'flex', width: 14, height: 42, background: ACCENT, borderRadius: 3, marginRight: 20 }} />
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', width: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', flexShrink: 0, marginLeft: 36 }}>
            ...
            <div style={{ display: 'flex', width: '100%', height: 6, background: ACCENT, borderRadius: 2, marginTop: 14 }} />
          </div>
        </div>
