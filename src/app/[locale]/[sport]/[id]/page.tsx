...
export default async function ThreadDetailPage({
  ...
  return (
    <article className="mx-auto max-w-prose">
      ...
      {/* 記事末尾＝読み終えた瞬間のカードCTA（②トリガー）。/player/{slug}#card で着地と同時にメーカーが
          開く（ループの着地＝GamelogAnalysis の #card ディープリンク）。MLB＋タグ選手があるときのみ。 */}
      {sport === 'mlb' && taggedPlayers.length > 0 && (
        <aside className="mt-10 border-t border-line pt-5">
          <p className="text-xs font-medium uppercase tracking-wider text-ink-soft">
            {t('player.cardCtaLead')}
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {taggedPlayers.map((p) => (
              <Link
                key={p.slug}
                href={`/player/${p.slug}#card`}
              ...
              >
                ...
                {t('player.cardCta', { name: p.nameJa })}
